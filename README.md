...
- 📡 订阅自动更新
- 🔗 独立的用户订阅接口
- 🚀 同时提供 Clash YAML 与 Shadowrocket Base64 用户订阅
- 📝 配置模板管理
- 🔗 链式代理支持（批量生成）
- 🔄 节点多分组支持
- 🗑️ 节点批量删除
- 🔌 3x-ui 后端对接与入站/客户端管理
- ⚡ 用户目录快捷新建 3x-ui 入站并自动挂载到用户订阅
- 🧭 支持按用户覆盖订阅节点的 IP/域名和端口，适配外部转发场景
- 📊 用户总流量限制与单个 3x-ui 入站节点限制分离管理
...
- 第一行是原有 Clash YAML 订阅，复制后导入 Clash Meta 客户端。
- 第二行是 Shadowrocket Base64 订阅，复制后在 Shadowrocket 中选择 `Subscribe` 并粘贴到 URL 栏。

两种链接使用同一个用户令牌并包含相同的可用节点；Shadowrocket 订阅会自动跳过链式代理和没有标准分享链接格式的节点。

### 5. 对接 3x-ui 后端
1. 点击"对接3x-ui后端"
2. 添加 3x-ui 面板地址、认证方式和订阅连接地址
3. 点击"检查 3x-ui API 能力"确认接口可用
4. 可在该页面同步入站、查看客户端、编辑或删除远端入站

### 6. 为用户挂载 3x-ui 节点
1. 点击"用户管理"
2. 在用户行点击"管理节点"
3. 可选择已有 3x-ui 入站并创建用户客户端
4. 也可点击"快捷新建入站"，按默认 `VLESS + TCP + 无 TLS + 随机端口` 快速创建
5. 创建成功后，该入站会立即进入当前用户的专属订阅

### 7. 覆盖用户订阅节点地址
在"用户管理"列表左侧点击三角展开用户节点详情，可以为单个用户节点设置订阅 IP/域名和订阅端口。

该覆盖只影响用户订阅中生成的实际连接地址，适用于外部端口转发、反代入口、隧道入口等场景；3x-ui 原入站不会被修改，流量统计仍然来自原始入站节点。
...
- 🔗 **独立订阅**：每个用户有独立的订阅链接
- 🚀 **多客户端格式**：同一用户可获取 Clash YAML 和 Shadowrocket Base64 订阅
- 📝 **模板管理**：自定义 Clash 配置模板
- 🔗 **链式代理**：支持创建多级代理节点
- 🔢 **节点排序**：自定义节点在订阅中的输出顺序
- ✏️ **节点编辑**：手动创建或编辑节点配置
...
### 3x-ui 对接功能
- 🔌 **多后端管理**：可配置多个 3x-ui 后端，并检测 API 可用性
- 🌐 **入站管理**：支持同步、查看、新建、编辑和删除 3x-ui 入站节点
- 👥 **用户客户端挂载**：为用户在指定入站下创建独立客户端，并自动进入用户订阅
- ⚡ **快捷新建入站**：在用户目录中快速创建可订阅协议入站，并同步创建用户客户端
- 🧭 **订阅地址覆盖**：支持按用户节点覆盖订阅输出的 IP/域名和端口，不影响远端入站
- 📊 **流量统计同步**：从 3x-ui 原始入站同步上行、下行、总流量、到期时间和重置策略
- 🧱 **母子限额分离**：用户总流量限制作为母限制；单个入站节点的总流量/到期/重置作为子限制
...
5. **3x-ui 用户目录说明**：
   - 用户总流量限制是母限制，显示在用户列表中，并作用于整个用户订阅
   - 3x-ui 入站节点的总流量、到期时间和重置策略是子限制，显示在用户展开的节点详情中
   - 订阅 IP/端口覆盖只改变用户订阅里的连接地址，不会修改 3x-ui 原入站端口
   - 流量统计以原始 3x-ui 入站为准；点击"同步流量"可立即刷新本地缓存
   - 用户访问订阅时，系统会按缓存过期策略自动同步 3x-ui 统计
...
### v2.2 (2026-06-29)
- ✅ 新增 3x-ui 后端对接、入站管理和用户客户端挂载
- ✅ 新增用户目录快捷新建 3x-ui 入站并自动进入用户订阅
- ✅ 新增用户节点订阅 IP/域名和端口覆盖，支持外部转发场景
- ✅ 区分用户总流量母限制和单个入站节点子限制
- ✅ 优化用户节点弹窗打开速度，支持本地缓存快速展示
- ✅ 删除用户目录远端节点时同步删除对应 3x-ui 入站及本地映射
